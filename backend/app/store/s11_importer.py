...
from app.core.s01_constants import SOURCE_USER
from app.core.s02_config import MODEL_PATH
from app.pipeline.s08_rules import apply_rules
from app.pipeline.s09_model import categorize_unmatched, load_model
from app.core.s04_models import Transaction, Upload
from app.pipeline.s07_parser import parse_statement


def find_known_fingerprints(session: Session, fingerprints, user_id=None):
    ...
    for start in range(0, len(fingerprint_list), batch_size):
        ...
        rows = session.execute(
            select(Transaction.fingerprint).where(
                Transaction.fingerprint.in_(batch),
                # Scoped to the caller: another user's statement is not this
                # user's duplicate, and they cannot see it to know why a row
                # went missing.
                *([Transaction.user_id == user_id] if user_id is not None else []),
            )
        ).scalars()
        known.update(rows)
    return known
...
def categorize(rows, model_path=None, user_rules=None):
    ...
    """
    automatic = [row for row in rows if row.get("category_source") != SOURCE_USER]

    apply_rules(automatic, user_rules=user_rules)
...
def import_statement(
    session: Session, filename: str, file_bytes: bytes, user_id=None,
    account_id=None,
) -> dict:
    ...
    known = find_known_fingerprints(
        session, {row["fingerprint"] for row in parsed_rows}, user_id=user_id
    )
    new_rows, duplicates = split_new_from_duplicates(parsed_rows, known)

    # The user's own rules, applied before the built-in ones and winning over
    # them — see s11a_rules for why that precedence is the right way round.
    from app.store.s11a_rules import active_pairs

    categorize(new_rows, user_rules=active_pairs(session, user_id) if user_id else None)
